...
import { getServiceDirectory, getServices } from "@/lib/content-pages";

const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://instadrop.uk";

export async function generateMetadata(): Promise<Metadata> {
  return contentMetadata(await getServiceDirectory(), "/services");
}

export default async function ServicesPage() {
  const [page, services] = await Promise.all([getServiceDirectory(), getServices()]);
  if (!page) notFound();
  const schema = {
