...
export async function generateMetadata({ params }: PageProps<"/industries/[slug]">): Promise<Metadata> { const { slug } = await params; const industry = industries.find((item) => item.slug === slug); return industry ? { title: `${industry.title} Courier Services`, description: industry.consignments, alternates:{canonical:`/industries/${slug}`} } : {}; }
