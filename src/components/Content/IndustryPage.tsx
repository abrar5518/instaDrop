...
export default function IndustryPage({ industry }: { industry: Industry }) {
  ...
  return <main className="bg-white">
    ...
    <section className="bg-[#0066ff] px-4 py-14 text-white sm:px-8"><div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 text-center lg:flex-row lg:text-left"><div><p className="text-xs font-bold uppercase tracking-widest text-white">Ready when you are</p><h2 className="mt-2 font-display text-3xl font-extrabold">Need a {industry.title.toLowerCase()} courier?</h2></div><Link href="/instant-quote" className="inline-flex items-center gap-2 rounded-full bg-[#c6ff00] px-7 py-3.5 text-sm font-extrabold text-[#0a192f]">Request your quote <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link></div></section>
  </main>;
}
